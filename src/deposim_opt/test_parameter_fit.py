...
def _search(
    ...
    *,
    ...
    return {
        ...
        "pruner": pruner,
        "sampler_options": {},
        "storage": storage or {"url": "", "study_name": "", "load_if_exists": False},
    }
...
class TestFitOptuna(unittest.TestCase):
        ...
        with TemporaryDirectory() as tmp:
            ...
            opt_spec = SimpleNamespace(
                ...
                parameter_fit=SimpleNamespace(
                    search=_search(2, 123),
                    fidelity={"levels": [1]},
                    objective={
                        "loss": {"name": "huber", "standardized": False, "delta_nm": 10.0},
                        "penalties": {
                            "lambda_solver": 0.1,
                            "lambda_prior": 0.0,
                        },
            ...
            for key in ["loss_data", "penalty_solver", "penalty_prior", "score_total"]:
        ...
        with TemporaryDirectory() as tmp:
            ...
            def build_spec(n_trials: int) -> SimpleNamespace:
                return SimpleNamespace(
                    measurement={"file": str(meas_path), "keys": {"h": "h_nm", "xy": "xy"}},
                    parameter_fit=SimpleNamespace(
                        search=_search(n_trials, 11, method="tpe", pruner="median", storage={"url": f"sqlite:///{db_path}", "study_name": study_name, "load_if_exists": True}),
                        fidelity={"levels": [1]},
                        objective={
                            "loss": {"name": "huber", "standardized": False, "delta_nm": 10.0},
                            "penalties": {
            ...
            out1 = fit_candidate_parameters(
                sim_spec=sim_spec,
                role_candidate=role,
                order_candidate=order,
                opt_spec=build_spec(1),
            )
            out2 = fit_candidate_parameters(
                sim_spec=sim_spec,
                role_candidate=role,
                order_candidate=order,
                opt_spec=build_spec(2),
            )
            self.assertGreaterEqual(len(out2["optimization_trace"]), len(out1["optimization_trace"]))
            self.assertGreaterEqual(len(out2["optimization_trace"]), 3)
    ...
    def test_run_fit_emits_topk_assignments(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            optimization_summary_path = run_dir / "tables" / "optimization_summary.csv"
            optimization_trace_path = run_dir / "tables" / "optimization_trace.csv"
            loss_components_path = run_dir / "tables" / "loss_components.csv"
            ...
            self.assertTrue(optimization_summary_path.exists())
            self.assertTrue(optimization_trace_path.exists())
            self.assertTrue(loss_components_path.exists())
