...
The goal is that **Codex CLI (gpt-5.3-codex, reasoning=high)** can run:
...
3. acquire `runs/autorun.lock` (single-runner guard; stale lock auto-recovery)
4. record task completion in `runs/autorun_state.json`
5. append the exact `codex exec` command line to `runs/autorun_commands.log` for reproducibility

Execution is strictly sequential (one task at a time in dependency order).
If you want manual checkpointing after every task, run:

```bash
./scripts/commands.sh step_next
```
...
### 4) Non-git workspace behavior

If the directory is not a git repository, autorun automatically adds:

- `codex exec --skip-git-repo-check` (when supported by installed CLI)

This behavior can be overridden with:

- `./scripts/run.sh --git-check strict` (fail unless inside git repo)
- `./scripts/run.sh --git-check skip`   (always request skip flag)
...
- One task step-run: `./scripts/commands.sh step_next`
- Autorun dry-run check: `./scripts/commands.sh verify_autorun`
- Task contract check: `./scripts/commands.sh verify_task_contracts`
- Milestone state sync: `./scripts/commands.sh reconcile_state --milestone P1,P2`
- Final P2 consistency gate: `./scripts/commands.sh verify_task P2-999`

Do not hardcode `python` vs `python3`. Preflight writes `scripts/env.sh` and `commands.sh` uses it.

Note on P2 verification:

- `verify_p2` runs implementation gates for `P2-001..P2-007`.
- `verify_task P2-999` is the final consistency gate and requires synchronized `runs/autorun_state.json`.
- After manual implementation/verification, run `reconcile_state --milestone P1,P2` before `verify_task P2-999`.
...
- Model gap triage: `docs/MODEL_GAP.md`
